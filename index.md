...
#### Karl Color
![colorStart.png](https://i.imgur.com/CRAvpQd.png)<br>Color image used as input for Part 1.<br>
...
2. Since the result of the face detection only returns a rough area of where it thinks the face is, we extend the detected face region by a substantial percentage so that the entire head is in the frame. At this point, we crop out the head region in the 1st and 10th images of the headshot sequence.<br><br>Below is what we have after the first two steps. The green box is the result of the harr cascade face detection. The blue box is the region that we crop out to use for the remaining steps.<br>
![colorFaceDetect.png](https://i.imgur.com/1vR4Jsn.png)
3. Then, we run Canny edge detector on the two headshot images. Canny returns a binary image where only strong edges are detected.<br>
![colorCanny.png](https://i.imgur.com/G8d2Xfc.png)
4. Next, we find the absolute value of the difference between the two Canny images. We assume that the person's head is in the foreground and everything in the background is much further away. The sequence of 10 headshots takes around 1 second to perform, so the likelyhood that the person moves (even slightly) is high. Because they are in the foreground of the image, the distance that they move (in terms of pixels) is larger than the distance that the background moves.<br>
![colorDiff.png](https://i.imgur.com/gOYAEsz.png)
5. Now, we perform a series of erosions and dilations on the image.<br>
![colorDilateErode.png](https://i.imgur.com/EVPMIeK.png)
6. Then we keep the largest object of the image (the head). This helps erase edges that were in the background of the image.
7. We perform more erosions and dilations on the image.<br>
![colorLargestObj.png](https://i.imgur.com/3gPnX3O.png)
8. Now that we have a single connected component, we are able to find the contours of the binary image. 
9. Using the contour points, we can fit an ellipse to the image.<br>
![colorContoursEllipse.png](https://i.imgur.com/No3qkEx.png)
10. Finally, we are able to crop out the ellipse of the face region of the 1st image.<br>
![colorResult.png](https://i.imgur.com/bBTHG01.png)

#### Thermal image:
1. Run harr cascade face detection on the first image.
2. Since the result of the face detection only returns a rough area of where it thinks the face is, we extend the detected face region by a substantial percentage so that the entire head is in the frame. At this point, we crop out the head region in the 1st image.<br><br>Below is what we have after the first two steps. The green box is the result of the harr cascade face detection. The blue box is the region that we crop out to use for the remaining steps.<br>
![irFaceDetect.png](https://i.imgur.com/8vXNDIb.png)
3. Now, we threshold the grayscale image with a simple threshold of [50,255].<br>
![irThreshold.png](https://i.imgur.com/0hun3nd.png)<br>
4. Next, we erode the thresholded image.<br>
![irErode.png](https://i.imgur.com/phakDdu.png)
...
7. Now that we have a single connected component, we are able to find the contours of the binary image.<br>
![irLargestObj.png](https://i.imgur.com/oActTTw.png)
8. Using the contour points, we can fit an ellipse to the image. This allows us to create a mask that is the shape of the ellipse.<br>
![irContourEllipse.png](https://i.imgur.com/UBuH1Lg.png)
9. Finally, we are able to crop out the ellipse of the face region of the 1st image.<br>
![irResult.png](https://i.imgur.com/8sO5cep.png)
...
- The thermal image will be the best way to predict thermal comfort. However, our thermal camera has an extremely small resolution. This poses the the biggest problem on our dataset. 
- The thermal image might only need the thresholding to find contours that will fit an ellipse. 
